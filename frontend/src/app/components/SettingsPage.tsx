...
export function SettingsPage({ user, onLogout, onUserUpdate }: Props) {
  ...
  useEffect(() => {
    if (isAdmin && !siteSettingsLoaded) {
      apiFetch('/api/v1/site-settings/')
        .then(r => r.json())
        .then(d => {
          setSystem(prev => ({
            ...prev,
            company_name:    d.site_name    || prev.company_name,
            company_email:   d.site_email   || prev.company_email,
            company_phone:   d.site_phone   || prev.company_phone,
            company_address: d.site_address || prev.company_address,
          }));
          if (d.site_logo_url) setSiteLogoPreview(d.site_logo_url);
  ...
  const saveSiteSettings = async () => {
    ...
    try {
      const fd = new FormData();
      fd.append('site_name',    system.company_name);
      fd.append('site_email',   system.company_email);
      fd.append('site_phone',   system.company_phone);
      fd.append('site_address', system.company_address);
      if (siteLogoFile) fd.append('site_logo', siteLogoFile);

      const res = await apiFetch('/api/v1/site-settings/', { method: 'PATCH', body: fd });
      if (!res.ok) { addToast('error', 'فشل حفظ إعدادات الموقع'); return; }
      addToast('success', '✅ تم حفظ إعدادات الموقع بنجاح');
    } catch { addToast('error', 'خطأ في الاتصال'); }
    finally { setSaving(false); }
  };
          ...
          {activeTab === 'system' && isAdmin && (
            <>
              ...
              <SectionCard title="معلومات الشركة" description="البيانات التي تظهر للعملاء والوكالات">
                <div className="space-y-4">
                  {/* شعار الموقع */}
                  <Field label="شعار الموقع الرئيسي">
                    <label className="block cursor-pointer">
                      <input type="file" accept="image/*" className="hidden"
                        onChange={e => {
                          const f = e.target.files?.[0] || null;
                          setSiteLogoFile(f);
                          if (f) { const r = new FileReader(); r.onload = e => setSiteLogoPreview(e.target?.result as string); r.readAsDataURL(f); }
                        }}/>
                      {siteLogoPreview
                        ? <div className="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-blue-300 hover:opacity-80 transition-opacity">
                            <img src={siteLogoPreview} className="w-full h-full object-cover" alt="logo"/>
                            <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 hover:opacity-100 transition-opacity">
                              <Camera className="w-6 h-6 text-white"/>
                            </div>
                          </div>
                        : <div className="w-24 h-24 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center hover:border-blue-400 hover:bg-blue-50 transition-colors">
                            <Globe className="w-8 h-8 text-gray-300 mb-1"/>
                            <span className="text-xs text-gray-400">رفع شعار</span>
                          </div>
                      }
                    </label>
